...
sumas = [sum(x) for x in lista_de_listas];
print(sumas)
...
tercerElemento = [listaElementos[2] for listaElementos in lista_de_tuplas];
...
listaTupla = [(lista.index(nombre), nombre) for nombre in lista]
print(listaTupla)
...
listComprehension = [apartamento[1] for apartamento in alquiler if apartamento[0] == 'Apartamento']
...
diccionario = {mes:gasto for mes, gasto in zip(meses, gasto)}
print(diccionario)
...
filtro2022M6000 = [venta for venta in ventas if venta[0]=='2022' and venta[1]>6000]
print(filtro2022M6000)
...
datosPacientes = [(("Hipoglicemia" if valor <= 70  else "Normal" if valor>70 and valor<99 else "Alterada" if valor>100 and valor<125 else "Diabetes"), valor) for valor in glicemia];
print(datosPacientes)
...
cantidades = [15, 12, 1, 15, 2, 11, 2, 12, 2, 4]
precios = [93.0, 102.0, 18.0, 41.0, 122.0, 14.0, 71.0, 48.0, 14.0, 144.0]
...
tablaVentas = [("id", "cantidad", "precio", "total")] 
tablaVentas += [(id, cantidad, precio, cantidad*precio) for id, cantidad, precio in zip(ids, cantidades, precios)];
print(tablaVentas)
...
estadosUnicos = list(set(estado[0] for estado in empleados));

EsadosListasEmpleados = {estado:[valor[1] for valor in empleados if valor[0]==estado] for estado in estadosUnicos}
EstadosTotalEmpleados = { estado: sum(listaEmpleados) for estado, listaEmpleados in EsadosListasEmpleados.items()}

print(EsadosListasEmpleados)
print(EstadosTotalEmpleados)